...
use routing::{Authority, Data, NodeBuilder, Request, Response, RoutingTable, XorName, EventStream};
use rust_sodium;
use std::env;
use std::path::Path;
...
impl Vault {
    ...
    pub fn run(&mut self) -> Result<bool, InternalError> {
        // FIXME(michael): might want to block here if no events available ATM.
    ...
    pub fn name(&self) -> XorName {
        unwrap!(self.routing_node.name()).clone()
    }
    ...
    fn on_request(&mut self,
        ...
        match (src, dst, request) {
            ...
            (src @ Authority::Client { .. },
             ...
             Request::Put(data, msg_id)) => self.maid_manager.handle_put(&mut self.routing_node, src, dst, data, msg_id),
            (src @ Authority::ClientManager(_),
             dst @ Authority::NaeManager(_),
             Request::Put(data, msg_id)) => self.data_manager.handle_put(&mut self.routing_node, src, dst, data, msg_id),
            // ================== Post ==================
            (src @ Authority::Client { .. },
             dst @ Authority::NaeManager(_),
             Request::Post(data, msg_id)) => self.data_manager.handle_post(&mut self.routing_node, src, dst, data, msg_id),
            // ================== Delete ==================
            (src @ Authority::Client { .. },
             dst @ Authority::NaeManager(_),
    ...
    fn on_response(&mut self,
        ...
        match (src, dst, response) {
            ...
            (Authority::ManagedNode(src_name),
             Authority::ManagedNode(_),
             Response::GetSuccess(data, _)) => self.data_manager.handle_get_success(&mut self.routing_node, src_name, data),
            // ================== GetFailure ==================
            (Authority::ManagedNode(src_name),
             Authority::ManagedNode(_),
            ...
            (Authority::NaeManager(_),
             ...
             Response::PutFailure { id, external_error_indicator, data_id }) => {
                self.maid_manager.handle_put_failure(&mut self.routing_node, id, data_id, &external_error_indicator)
            }
            // ================== Invalid Response ==================
