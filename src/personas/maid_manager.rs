...
impl MaidManager {
    ...
    pub fn handle_put_success(&mut self,
        ...
        match self.request_cache.remove(&msg_id) {
            Some((src, dst)) => {
                ...
                let client_name = utils::client_name(&src);
                self.send_refresh(routing_node, &client_name,
    ...
    pub fn handle_refresh(&mut self, routing_node: &mut RoutingNode, serialised_msg: &[u8]) -> Result<(), InternalError> {
        match serialisation::deserialise::<Refresh>(serialised_msg)? {
            Refresh::Update(maid_name, account) => {
                if let None = routing_node.close_group(maid_name, GROUP_SIZE) {
                    return Ok(());
                }
    ...
    pub fn handle_node_added(&mut self,
        ...
        for (maid_name, account) in &self.accounts {
            self.send_refresh(routing_node, maid_name, account, MessageId::from_added_node(*node_name));
        }
    }

    pub fn handle_node_lost(&mut self, routing_node: &mut RoutingNode, node_name: &XorName) {
        for (maid_name, account) in &self.accounts {
            self.send_refresh(routing_node, maid_name, account, MessageId::from_lost_node(*node_name));
        }
    }

    fn send_refresh(&self, routing_node: &mut RoutingNode, maid_name: &XorName, account: &Account, msg_id: MessageId) {
